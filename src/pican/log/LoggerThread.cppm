...
private:  // constructor
    LoggerThread(LogLevel level, ThreadName name, ds::Array<log::Buffer> buffers, ds::Array<log::Sink> sinks) :
        level_f{level}, buffers_f{buffers}, buffersCount_f{0}, sinks_f{sinks}, eventfd_f{sync::EventFd::Mode::NOTIFY},
                ...
                while (toPop > 0) {
                    ...
                    for (Sink& sink : self->sinks_f) {
                        ...
                        fmt::format_to_n_result<char*> formatted = fmt::format_to_n(
                            ...
                            log_level_to_string(entry.level()), threadIdentity.name, threadIdentity.id,
                            entry.message_buffer()
                        );
