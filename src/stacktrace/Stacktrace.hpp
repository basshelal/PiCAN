...
struct Entry {
    ...
    int lineNumber = 0;
    std::array<char, 128> symbolName = {};
    std::array<char, 128> functionName = {};
...
namespace stacktrace::_ {
// Global state required by libbacktrace, inline so that there is exactly one instance across the whole program
inline struct backtrace_state* libbacktraceState_g = nullptr;
...
syminfo_callback(
    ...
    if (symbolName != nullptr) {
        ...
        ::memcpy(entry.symbolName.data(), symbolName, length);
    }
...
full_callback(void* data, uintptr_t programCounter, const char* filename, int lineNumber, const char* functionName) {
    ...
    if (functionName != nullptr) {
        const std::size_t length = std::min(entry.functionName.size(), std::strlen(functionName));
        ::memcpy(entry.functionName.data(), functionName, length);
    }

    if (filename != nullptr) {
        const std::size_t length = std::min(entry.filename.size(), std::strlen(filename));
        ::memcpy(entry.filename.data(), filename, length);
    }
...
initialize(char** argv) {
    if (_::libbacktraceState_g != nullptr) {
        return;
    }
    ...
    _::libbacktraceState_g = ::backtrace_create_state(programPath, 1, _::error_callback, nullptr);
    ...
    ::backtrace_full(_::libbacktraceState_g, 0, _::empty_callback, _::error_callback, nullptr);
}
...
get_stacktrace(EntryCallback callback, void* userData = nullptr, int skipFrames = 0) {
    ...
    if (_::libbacktraceState_g == nullptr) {
        const _::FdWriter writer{STDERR_FILENO};
        writer.write_string("Backtrace state not initialized!\n");
        return;
    }
    _::CallbackInfo info{.entry = Entry{}, .callback = callback, .index = 0, .userData = userData};
    ::backtrace_full(_::libbacktraceState_g, skipFrames + 1, _::full_callback, _::error_callback, &info);
}
...
print_entry(const Entry& entry, std::FILE* file) {
    const _::FdWriter writer{::fileno(file)};
    writer.write_string(entry.filename.data());
    writer.write_string(":");
    writer.write_int(entry.lineNumber);
    writer.write_string(" @ ");
    writer.write_string(entry.functionName.data());
    writer.write_string(" ");
    writer.write_string(entry.symbolName.data());
    writer.write_string(" @ ");
